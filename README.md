...
├── TouchIDSudoConfigurator — pure state parser for /etc/pam.d/sudo_local + biometry check (LocalAuthentication, with an IOKit hardware-presence fallback); renders an idempotent shell command to atomically (mktemp + mv) install `auth sufficient pam_tid.so`, invoked via osascript with administrator privileges from InfoView
