...
5. **Act** — each update source drives its own action: brew casks run `brew install --cask` with a live log panel; JetBrains apps open Toolbox; GitHub apps open the Releases page; Sparkle apps prompt inside the app itself; self-updating apps whose cask lags (Antigravity, ChatGPT, Postman, Parallels, Discord, Signal, Chrome, Obsidian) are launched so their own updater takes over (never routed through brew, which would reinstall the stale cask); Creative Cloud applications open Adobe's installed client (web page only as a fallback), the only thing that can install an Adobe update; npm globals are bumped with `npm install -g <pkg>@latest`.
