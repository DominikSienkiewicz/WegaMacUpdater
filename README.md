...
| 1 | Adobe Creative Cloud catalog | Creative Cloud applications (Lightroom, Photoshop, Illustrator, InDesign, Premiere Pro…). Adobe ships no Sparkle appcast and no Squirrel feed, and — Acrobat Reader aside — Homebrew packages none of these apps, so every other source is silent and an outdated Lightroom was reported by nothing. The installed product is read from Creative Cloud's own `/Library/Application Support/Adobe/Uninstall/*.adbarg` records, which name the SAP code (`LRCC`), the version and the product name together, so no hand-maintained bundle-id table can go stale; the available version is the newest entry for that SAP code in Adobe's published product catalog, fetched once per scan and cached for 24 h. Ranked **below** Homebrew Cask on purpose: where a cask exists, `brew` can apply the update in place while this source can only open Creative Cloud, which is the only thing that installs an Adobe update |
