...
- **Build & Test** — `swift build --build-tests` + `swift test`.
...
- **SonarCloud** — runs the SonarQube/SonarCloud scanner against `sonar-project.properties`. Skipped until a `SONAR_TOKEN` secret is configured, so it never blocks CI before setup. Outbound URLs are sourced from `endpoints.json` via `AppEndpoints`, so `S1075` ("hard-coded URI") only fires on genuinely configurable endpoints; tests and the fixed-system-path files are excluded in the properties file.
