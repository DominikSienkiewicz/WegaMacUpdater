...
The package targets the **Swift 6 language mode** (`swift-tools-version: 6.0`), so the whole codebase compiles under strict concurrency checking. Every push and pull request to `main` runs three GitHub Actions jobs (`.github/workflows/ci.yml`) on a `macos-15` runner with the latest stable Xcode:
...
- **Package** — runs `scripts/build-pkg.sh` to prove the whole packaging path composes, asserts the binary is **universal** (arm64 + x86_64) and that `app-catalog.json` is bundled, and uploads the resulting `.pkg` as an artifact.
